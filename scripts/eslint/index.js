...
function runESLintOnFilesWithOptions(filePatterns, onlyChanged, options) {
  ...
  if (onlyChanged && changedFiles === null) {
    // Calculate lazily.
    changedFiles = _toConsumableArray(
      (0, _listChangedFiles.listChangedFiles)(),
    );
  }

  var finalFilePatterns = onlyChanged
...
function intersect(files, patterns) {
  ...
  patterns.forEach(function(pattern) {
    intersection = _toConsumableArray(intersection).concat(
      _toConsumableArray(
...
function runESLint(_ref) {
  ...
  var _runESLintOnFilesWith = runESLintOnFilesWithOptions(
      allPaths,
      onlyChanged,
    ),
